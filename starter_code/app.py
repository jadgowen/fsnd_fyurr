...
class Venue(db.Model):
    ...
    show_ids = db.relationship('Shows',backref='venue', lazy=True)
...
class Artist(db.Model):
    ...
    seeking_description = db.Column(db.String(1000))
    show_ids = db.relationship('Shows',backref='artist', lazy=True)
      ...
      for venue in venues: # Building individual library
          if (venue.city == ven[0]) and (venue.state == ven[1]): # If venue city and state match
              ...
              for shows in show_data:
                  if shows.show_time > current_time:
                      num_shows += 1
...
def search_venues():
  ...
  ven_list = []
  for venue in venues:
      ven_shows = Shows.query.filter_by(venue_id=venue.id).all()
...
def search_artists():
  ...
  art_list = []
  for artist in artists:
      art_shows = Shows.query.filter_by(artist_id=artist.id).all()
      num_upcoming_shows = 0
...
def create_show_submission():
  ...
  show_time = format_datetime(str(form.start_time.data))
  new_show = Shows(artist_id=artist_id,venue_id=venue_id,show_time=show_time)
  try:
